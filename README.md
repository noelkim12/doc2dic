...
| `init`, `status`, `config`, `concept`, `variant`, `review`, `check`, `analyze`, `graph`, `serve` 명령 그룹 | Current | CLI 명령 그룹이 제공됩니다. |
...
| MCP stdio 서버 | Current | `doc2dic serve --mcp`로 OpenCode, Claude Code 같은 도구에 glossary context를 연결합니다. |
...
MCP에서 후보 용어 추출 LLM은 기본 책임이 아닙니다. Claude Code, OpenCode, Codex 같은 호출 harness가 문서를 읽고 LLM 추론으로 후보를 뽑고, Doc2Dic MCP는 기존 용어집 context, tag suggestion, stale/index guidance, embedding-backed lookup 경계를 제공합니다. 레거시 로컬 분석 도구 `doc2dic_analyze`는 기본 노출되지 않으며, 과거 CLI 분석 흐름 검증이 필요할 때만 `DOC2DIC_MCP_TOOLS=analyze`로 명시 opt-in합니다.
...
`doc2dic serve --mcp`는 현재 기능입니다. 로컬 API와 웹 개발 서버를 함께 띄울 때는 `doc2dic serve web --path <project>`를 씁니다. 공개 원격 `curl`과 `sh` 조합 설치는 현재 기능이 아니라, 패키징과 호스팅이 갖춰졌을 때의 조건부 경로입니다.
...
## embedding provider 설정과 auth 파일

`doc2dic config`는 프로젝트 설정을 바꾸는 canonical 명령입니다. 인자 없이 실행하면 prompt로 embedding provider, 모델명, API key 입력을 안내합니다.
...
provider와 모델명은 프로젝트의 `.doc2dic/glossary.sqlite3` settings에 저장됩니다. 입력한 API key는 프로젝트 안이 아니라 사용자 전역 auth 파일에 저장됩니다. 기본 위치는 Linux/macOS에서 `~/.config/doc2dic/auth.json`, Windows에서 `%APPDATA%/.config/doc2dic/auth.json`입니다. `XDG_CONFIG_HOME`이나 `DOC2DIC_AUTH_FILE`을 설정하면 해당 위치를 우선합니다.

Voyage provider를 선택하면 명시 모델이 없을 때 기본 모델은 `voyage-4-large`입니다. 런타임은 프로젝트 settings의 provider와 model을 먼저 읽고, API key는 저장소나 SQLite가 아닌 아래 순서로 찾습니다.

1. `VOYAGE_API_KEY`
2. `DOC2DIC_EMBEDDING_API_KEY`
3. `DOC2DIC_AUTH_FILE` 또는 기본 auth 파일의 `embedding.api_keys.voyage`

`doc2dic config embedding`으로 입력한 key는 사용자 전역 auth 파일에만 저장됩니다. `.doc2dic/glossary.sqlite3`에는 provider와 model만 저장하며 raw key는 저장하지 않습니다.

CLI 출력과 `doctor`는 API key 값을 표시하지 않고 저장 여부만 보여줍니다.

기본 테스트와 `scripts/test.sh --smoke`는 Voyage key나 네트워크를 요구하지 않습니다. 실제 Voyage 호출 확인은 `DOC2DIC_RUN_LIVE_VOYAGE_TESTS=1`과 지원되는 key가 모두 있을 때만 선택적으로 실행합니다. 이 live smoke는 status, model, embedding count, vector dimension, token usage 같은 비밀이 아닌 메타데이터만 출력해야 합니다.
...
OpenCode, Claude Code 같은 MCP 클라이언트에서 프로젝트 용어집 context를 읽으려면 stdio 서버를 실행합니다.
...
MCP 서버도 같은 `.doc2dic/glossary.sqlite3`를 읽습니다. 기본 MCP 표면은 harness가 추출한 후보를 용어집과 비교하도록 돕는 context 도구입니다. MCP 서버가 별도 LLM 후보 추출 provider를 자동 호출하지 않습니다. stale banner는 문서가 pending, stale, missing 상태일 수 있음을 알려주는 참고 알림입니다. 이 배너는 용어집 변경 승인도 아니고, 리뷰 큐 결정을 대체하지도 않습니다.

## 웹 UI 실행

이미 `.doc2dic/glossary.sqlite3`가 있는 프로젝트의 DB 정보를 웹으로 보려면 아래 명령을 실행합니다.

```bash
doc2dic serve web --path <project>
```
...
이 명령은 FastAPI API 서버를 `http://127.0.0.1:8765`에 띄우고, 저장소의 `web` 패키지에서 Vite 개발 서버를 `http://127.0.0.1:5173`에 띄웁니다. 종료할 때는 실행 중인 터미널에서 `Ctrl+C`를 누르면 두 프로세스를 함께 정리합니다.
...
| `doc2dic config` | Current | prompt로 embedding provider, 모델명, API key 저장을 설정합니다. |
| `doc2dic config embedding` | Current | embedding 설정 prompt를 바로 실행합니다. |
| `doc2dic config embedding doctor` | Current | embedding 설정과 auth 파일 상태를 redacted 출력으로 확인합니다. |
...
| `DOC2DIC_RUN_LIVE_VOYAGE_TESTS=1 ... live_voyage` | Optional | 명시 opt-in과 지원되는 Voyage key가 있을 때만 live smoke를 실행합니다. 기본 테스트에는 필요하지 않습니다. |
...
| `doc2dic serve web` | Current | 현재 프로젝트의 API 서버와 웹 개발 서버를 함께 시작합니다. |
| `doc2dic serve web --path <project>` | Current | 지정한 프로젝트의 DB를 API와 웹 UI에서 볼 수 있게 시작합니다. |
...
| `doc2dic serve` | Current | 실행 표면을 명시하라는 안내를 출력합니다. 웹은 `doc2dic serve web`, MCP는 `doc2dic serve --mcp`를 씁니다. |
...
3. `docs/workflow-study-guide.md`: 코드 진입점과 파일별 학습 흐름을 설명합니다.
4. `docs/opencode-workflow.md`: 이 저장소의 에이전트 소유권 흐름을 설명합니다.
5. `docs/graphify-integration.md`: Graphify 내보내기 경계와 연동 흐름을 설명합니다.
6. `docs/post-mvp.md`: 뒤로 미룬 통합과 비범위를 정리합니다.
